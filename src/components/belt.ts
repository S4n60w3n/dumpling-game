...
export class Belt {
  ...
  checkRecycle() {
    const image = (this.container.getAt(this.recycleIndex) as Phaser.GameObjects.Image)
    const x = image.x + this.container.x
    if (x > SCREEN_WIDTH) {
      const firstImage = (this.container.getAt(((this.recycleIndex + 1) % AMOUNT)) as Phaser.GameObjects.Image)
